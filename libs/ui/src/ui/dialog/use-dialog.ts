...
export function useMorphDialog(options: MorphDialogOptions = {}) {
  ...
  const refs = React.useMemo<MorphDialogRefs>(
    ...
    [setTrigger, setTriggerHost, setDialog, setBackdrop, setDrag, setSurface, setWindow]
  )
  ...
  const fullscreen = React.useSyncExternalStore(subscribeFullscreen, readFullscreen, () => query === ALWAYS)
  ...
  const flipId = "dialog-" + React.useId().replace(/[^a-zA-Z0-9_-]/g, "")
  ...
  const log = React.useCallback((label: string, data?: unknown | (() => unknown), throttleMs = 0) => {
    ...
    console.log(`%c[dialog]%c ${label}`, "color:#09ee61;font-weight:600", "color:inherit", payload ?? "")
  }, [])
  ...
  const fitFrame = React.useCallback(() => {
    ...
    log(
      ...
      () => ({
        ...
        ...(radius === undefined ? {} : { radius: Math.round(radius * 10) / 10 })
      }),
      120
    )
  }, [log])
  ...
  const settle = React.useCallback(() => {
    ...
    thawChild()
    gsap.set(surface, {
      clearProps: "position,margin,left,top,width,height,backgroundColor,borderRadius"
    })
  ...
  const build = React.useCallback(() => {
    ...
    return tlRef.current
  }, [flipId, freezeChild, fitFrame, getTrigger, log, settle, fullscreen, concealTrigger])
  ...
  }, [bakeDrag, freezeChild, thawChild, fitFrame, getTrigger, log, open, revealTrigger, unlockScroll])
  ...
  const onPointerMove = React.useCallback((e: React.PointerEvent<HTMLElement>) => {
    ...
    gsap.set(dragEl, {
      x: d.dx,
      y: d.dy,
      scale: Math.max(1 - Math.hypot(d.dx, d.dy) / optsRef.current.dragFalloff, 0.3)
    })
  }, [])
  ...
  const onPointerUp = React.useCallback(
    (e: React.PointerEvent<HTMLElement>) => {
      ...
      const o = optsRef.current
      const dismissed = dist > o.dismissDistance || (speed > o.dismissSpeed && dist > 20)
      log("drag end", {
